...
// Spritesheet & Images
PImage spriteSheet, spriteSheet2;
...
PImage[][] p2Sprites = new PImage[4][2];
PImage[] p2popSprites = new PImage[15];
...
void loadSprites() {
  ...
  spriteSheet2 = loadImage("p2sprites.png");
  ...
  for(int i=0;i<4;i++) {
    for(int j=0;j<2;j++) {
      p2Sprites[i][j]=spriteSheet2.get(20*j, 20*order[i], 16, 16);
    }
  }
  for(int i=0;i<4;i++) {
    for(int j=0;j<3;j++) {
      p2popSprites[4*j+i]=spriteSheet2.get(40, 20*i, 16, 16); 
    }
  }
  p2popSprites[12]=p2popSprites[13]=p2popSprites[14]=popSprites[11];
}
void loadConstants() {
  loadSprites();
}